...
def create_test_spout(client, test_name):
    ...
    client.create_pipeline(
        pipeline_repo_name,
        transform=python_pachyderm.Transform(cmd=["sh"], image="alpine", stdin=[
            "while [ : ]",
            "do",
            "sleep 2",
            "date > date",
            "pachctl put file {}@master ./date*".format(pipeline_repo_name),
            "done"
            ],
